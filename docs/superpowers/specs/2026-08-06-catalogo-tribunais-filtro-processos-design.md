...
- Criar um endpoint autenticado e leve no backend para listar tribunais suportados e o sistema processual utilizado por cada um.
...
O backend exporá `GET /tribunals`, protegido pela autenticação já usada pelas demais rotas. A resposta será um objeto com a propriedade `tribunals`, contendo os códigos-base suportados e o sistema processual de cada tribunal:
...
Os códigos do catálogo serão validados contra o enum `Tribunal` do Prisma. Os sufixos `G1` e `G2` serão removidos e os valores repetidos serão consolidados. Um catálogo central do backend associará cada código-base ao sistema processual e será reutilizado pela rota de status, evitando duas fontes de metadados. A resposta não dependerá de processos, credenciais ou jobs existentes para o usuário.
...
A página de processos buscará o catálogo no servidor e passará as opções para `ProcessFilterControls`, tanto na variante desktop quanto na mobile. Cada opção continuará sendo um checkbox identificado pelo código do tribunal e a seleção continuará sendo aplicada automaticamente. O rótulo seguirá o formato `TJAM · Projudi`, exibindo o sistema processual sem alterar o valor `TJAM` enviado no parâmetro `tribunal`.
...
1. renderizar as opções e o sistema processual de cada tribunal no painel de filtros;
...
Se o catálogo não puder ser carregado por falha de rede ou resposta inválida, a carteira continuará acessível. O front usará como fallback os nove tribunais-base suportados no momento da implementação, com seus respectivos sistemas:

`STJ` (CPE), `TJAM` (Projudi), `TJBA` (PJe), `TJDFT` (PJe), `TJPI` (PJe), `TJRN` (PJe), `TRF1` (PJe), `TRF2` (e-Proc) e `TRF3` (PJe).
...
- Um catálogo central do backend determina quais tribunais são suportados, seus sistemas processuais e transforma os valores do enum em códigos-base.
...
- cada tribunal possui um sistema processual válido e o endpoint retorna o mapeamento esperado;
- a rota de status e `GET /tribunals` usam o mesmo catálogo de metadados;
...
- O filtro informa o sistema processual de cada tribunal e continua enviando somente o código-base na URL.
- A lista exibida e a validação da URL usam o mesmo catálogo.
- A adição futura de um tribunal exige somente seu registro no catálogo central do backend, associado ao enum e ao sistema processual; não exige alteração no front nem duplicação manual por grau.
